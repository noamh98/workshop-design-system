...
4. תרשימים: donut / bar / line דרך `data-chart` + `data-chart-config` בלבד.
...
| V6 | קווי יד רק דרך `data-sketch` מהרשימה; תרשימים רק `data-chart="donut\|bar\|line"` + `data-chart-config` JSON תקין | שגיאה |
...
4. הרחבת charts.js (bar-h RTL, stacked, gantt, waterfall) לפי ה-AUTO-TASK
   הקיים ב-`project/CLAUDE.md`, וחשיפתם למתכונים.
