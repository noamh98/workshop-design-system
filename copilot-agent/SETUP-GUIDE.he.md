...
| `knowledge/anti-patterns.md` | מה *לא* לעשות — "חלש מול טוב" לכל טעות נפוצה |
...
## 8. בקרת איכות מחוץ ל-Copilot Studio (מומלץ)

Copilot Studio לא מריץ קוד, ולכן ההוראות כוללות שלב SELF-CHECK שהמודל
מבצע על עצמו. לבדיקה אובייקטיבית יש בריפו (וב-ZIP, תחת `tools/`) שני
סקריפטים שמריצים מקומית על כל deck שהסוכן יצר:

```bash
node tools/validate-deck.mjs decks/my-deck.he.html   # חוקי סגנון, ללא תלויות
cd tools && npm install                               # חד-פעמי
node render-check.mjs ../decks/my-deck.he.html --out /tmp/rc   # צילום ובדיקת כל שקף
```

מצאת הפרה? הדבק את הפלט בצ'אט של הסוכן ובקש תיקון — זו לולאת ה-QA.

## 9. בדיקת קבלה (עשה את זה לפני שימוש אמיתי)
